...
## 6. Comparison with Sui

Against an equivalent Sui-testnet NFT-marketplace benchmark (same operations,
same sequential CLI-style methodology):

| Operation | Sui latency | **Solana latency** | Speed-up |
|-----------|------------:|-------------------:|---------:|
| mint | 1,985 ms | **749 ms** | ~2.6× faster |
| list | 2,295 ms | **944 ms** | ~2.4× faster |
| delist / cancel | 2,359 ms | **878 ms** | ~2.7× faster |
| Sequential TPS | 0.486 | **≈ 1.17** | ~2.4× higher |

On the same sequential methodology, the Solana marketplace shows **~2.4–2.7×
lower latency** across operations and **~2.4× higher** sequential throughput.
Sui's purchase path (347 ms) is notably fast due to its owned-object transfer
model; a like-for-like Solana `purchase_nft` measurement (two-wallet) is the
recommended next data point.
...
- **Listing is the heaviest operation** in both time and compute, because it
  ...
  transfer). This is intrinsic to an escrow marketplace and is consistent across
  blockchains.
...
compute**. On an identical sequential methodology the marketplace is **~2.4–2.7×
faster than an equivalent Sui deployment**, and the figures validate the system
as a realistic, low-cost, real-world NFT marketplace.
